...
export function MasaExperience() {
  ...
  useEffect(() => { let frame=0; const paint=()=>{const p=progress.current.current,clamp=(v:number)=>Math.max(0,Math.min(1,v)),dark=clamp((p-ENVIRONMENT.darkStart)/(ENVIRONMENT.darkFull-ENVIRONMENT.darkStart)),light=clamp((p-ENVIRONMENT.lightStart)/(ENVIRONMENT.lightFull-ENVIRONMENT.lightStart)),sand=clamp((p-ENVIRONMENT.darkStart)/(.50-ENVIRONMENT.darkStart))*(1-clamp((p-.50)/(.08))),lerp=(a:number,b:number,t:number)=>Math.round(a+(b-a)*t),base=ENVIRONMENT.intro.map((v,i)=>lerp(v,ENVIRONMENT.transition[i],sand)),bg=base.map((v,i)=>lerp(v,ENVIRONMENT.inside[i],dark*(1-light))),ink=ENVIRONMENT.ink.map((v,i)=>lerp(v,ENVIRONMENT.cream[i],dark*(1-light)));if(stage.current){stage.current.style.setProperty("--stage-bg",`rgb(${bg[0]} ${bg[1]} ${bg[2]})`);stage.current.style.setProperty("--stage-ink",`rgb(${ink[0]} ${ink[1]} ${ink[2]})`)}frame=requestAnimationFrame(paint)};frame=requestAnimationFrame(paint);return()=>cancelAnimationFrame(frame)},[progress]);
  return (
