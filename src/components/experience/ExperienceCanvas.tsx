...
import {Color,DirectionalLight,MathUtils,PointLight,Vector3} from "three";
...
const warmLight=new Color("#f3c477"),neutralLight=new Color("#f0d4aa"),technicalLight=new Color("#d8e0dc"),heatLight=new Color("#f2aa62"),mixedLight=new Color();
const warmBackground=new Color("#100b07"),neutralBackground=new Color("#0d0b09"),technicalBackground=new Color("#090c0d"),mixedBackground=new Color();
...
function Director({bridge,keyLight,fillLight}:{bridge:ExperienceBridge;keyLight:React.RefObject<DirectionalLight|null>;fillLight:React.RefObject<PointLight|null>}){
 ...
 useFrame((_,delta)=>{const store=bridge.progress.current;store.current=MathUtils.damp(store.current,store.target,store.reduced?20:12,delta);const p=store.current,mobile=size.width<720,pose=storyPose(p,mobile),history=1-smooth(range(p,PRE_ANATOMY.start,PRE_ANATOMY.end)),anatomy=smooth(range(p,.425,.45)),micro=(smooth(range(p,UPPER_ISOLATE.start,UPPER_ISOLATE.end))*(1-smooth(range(p,PISTACHIO_SOLO.start-.01,PISTACHIO_SOLO.start))))+smooth(range(p,LOWER_ISOLATE.start,LOWER_ISOLATE.end)),index=getInspectionIndex(p);
  ...
  const regional=smooth(range(p,.14,.22)),technical=smooth(range(p,.27,.405));mixedLight.copy(warmLight).lerp(neutralLight,regional).lerp(technicalLight,technical);mixedBackground.copy(warmBackground).lerp(neutralBackground,regional).lerp(technicalBackground,technical);scene.background=mixedBackground;
  const heat=smooth(range(p,.28,.32))*(1-smooth(range(p,.345,.37)));if(keyLight.current){keyLight.current.intensity=2.25+heat*.95+anatomy*.45;keyLight.current.color.copy(mixedLight).lerp(heatLight,heat)}if(fillLight.current)fillLight.current.intensity=.7+heat*.45;
 });return null
}
