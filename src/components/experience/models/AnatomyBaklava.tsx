...
import {treatAsBakedFilo} from "./materialTreatment";
...
export function AnatomyBaklava({bridge}:{bridge:ExperienceBridge}){
 ...
 const prepared=useMemo(()=>{const scene=SkeletonUtils.clone(gltf.scene);scene.traverse(child=>{if(!(child instanceof Mesh))return;if(!expectedMeshes.includes(child.name as MeshName)){child.visible=false;return}const array=Array.isArray(child.material),source=array?child.material:[child.material],materials=source.map((m:Material)=>{const c=m.clone();c.transparent=true;if(child.name!=="Pistachio")treatAsBakedFilo(c);return c});child.material=array?materials:materials[0];child.castShadow=true;child.receiveShadow=true;child.frustumCulled=false});
 ...
 useFrame((_,delta)=>{const group=root.current;if(!group)return;const p=bridge.progress.current.current,opacity=anatomyOpacity(p,bridge.ready.current.anatomy),activeIndex=getInspectionIndex(p),inspection=activeIndex>=0?inspections[activeIndex]:null,local=inspectionLocal(p),focus=inspection?smooth(range(local,.06,.2))*(1-smooth(range(local,.8,.96))):0,upperOnly=p>=UPPER_ISOLATE.start&&p<PISTACHIO_SOLO.start,pistOnly=p>=PISTACHIO_SOLO.start&&p<LOWER_ISOLATE.start,lowerOnly=p>=LOWER_ISOLATE.start&&p<GROUP_RETURN.start,returning=p>=GROUP_RETURN.start&&p<BEAUTY.end,rebuilding=p>=RECONSTRUCTION.start;
  ...
  prepared.meshes.forEach((mesh,name)=>{const original=prepared.originals.get(name)!;let y=finalY[name]*stagedExplosion(name,p),x=0,visibility=1,active=false;
   if(inspection?.phase==="macro"){active=inspection.key===name||(inspection.key==="UpperFilo3"&&upperFilo.includes(name))||(inspection.key==="LowerFilo3"&&lowerFilo.includes(name));visibility=active?1:.3;x=active?(inspection.station==="left"?.18:-.18)*focus:0}
   ...
   if(lowerOnly){const index=lowerFilo.indexOf(name);visibility=index>=0?1-lowerSpread*.12:1-lowerIsolation;if(index>=0)y=mix(finalY[name],(2-index)*.115,lowerIsolation)*lowerSpread;active=inspection?.phase==="lower"&&inspection.key===name;if(index>=0&&!active&&inspection?.phase==="lower")visibility=.3}
