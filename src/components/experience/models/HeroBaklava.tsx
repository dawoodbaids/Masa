...
function prepare(source:Object3D){const scene=SkeletonUtils.clone(source);scene.traverse(child=>{if(!(child instanceof Mesh))return;child.castShadow=true;child.receiveShadow=true;const array=Array.isArray(child.material),materials=array?child.material:[child.material],cloned=materials.map((m:Material)=>{const c=m.clone();c.transparent=true;if(child.name!=="Pistachio")treatAsBakedFilo(c);return c});child.material=array?cloned:cloned[0]});const box=new Box3().setFromObject(scene),size=box.getSize(new Vector3()),center=box.getCenter(new Vector3()),scale=MODEL_SCALE/Math.max(size.x,size.y,size.z);scene.scale.setScalar(scale);scene.position.copy(center).multiplyScalar(-scale);return scene}
